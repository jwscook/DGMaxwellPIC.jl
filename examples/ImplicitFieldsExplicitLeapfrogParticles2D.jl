...
function foo()
  ...
  dtc = norm((b .- a)./(NX, NY)) / DGMaxwellPIC.speedoflight
  dt = dtc / (OX^2 + OY^2)
...
##      @timeit to "stepfields" stepfieldsEuler!(u, A, k1, work, dt, S)
      ...
      #@timeit to "implicit fields" begin
      #  @timeit to "CN+ multiply" mul!(work, CN⁺, u) # do implicit timestep from n to n+1
      #  @timeit to "Add current" @tturbo @. work += S * dt
      #  @timeit to "luCN- solve" ldiv!(u, luCN⁻, work)
      #end
